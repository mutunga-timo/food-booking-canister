...
struct StableBTreeMap0ValueType(FoodBooking);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
#[ic_cdk_macros::query(name = "getFoodBookings")]
#[candid::candid_method(query, rename = "getFoodBookings")]
async fn _cdk_user_defined_getFoodBookings() -> (_AzleResult<Vec<FoodBooking>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getFoodBookings".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
...
#[ic_cdk_macros::query(name = "getFoodBooking")]
#[candid::candid_method(query, rename = "getFoodBooking")]
async fn _cdk_user_defined_getFoodBooking(
    _cdk_user_defined_id: String,
) -> (_AzleResult<FoodBooking, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getFoodBooking".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getFoodBooking", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getFoodBooking"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::query(name = "searchFoodBookings")]
#[candid::candid_method(query, rename = "searchFoodBookings")]
async fn _cdk_user_defined_searchFoodBookings(
    _cdk_user_defined_keyword: String,
) -> (_AzleResult<Vec<FoodBooking>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "searchFoodBookings".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("searchFoodBookings", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "searchFoodBookings"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_keyword
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::query(name = "countFoodBookings")]
#[candid::candid_method(query, rename = "countFoodBookings")]
async fn _cdk_user_defined_countFoodBookings() -> (_AzleResult<_CdkFloat64, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "countFoodBookings".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("countFoodBookings", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "countFoodBookings"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getFoodBookingsPaginated")]
#[candid::candid_method(query, rename = "getFoodBookingsPaginated")]
async fn _cdk_user_defined_getFoodBookingsPaginated(
    _cdk_user_defined_page: _CdkFloat64,
    _cdk_user_defined_pageSize: _CdkFloat64,
) -> (_AzleResult<Vec<FoodBooking>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getFoodBookingsPaginated".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getFoodBookingsPaginated", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getFoodBookingsPaginated"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_page.try_into_vm_value(&mut boa_context)?,
                            _cdk_user_defined_pageSize
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::query(name = "getFoodBookingsByTimeRange")]
#[candid::candid_method(query, rename = "getFoodBookingsByTimeRange")]
async fn _cdk_user_defined_getFoodBookingsByTimeRange(
    _cdk_user_defined_startTime: u64,
    _cdk_user_defined_endTime: u64,
) -> (_AzleResult<Vec<FoodBooking>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getFoodBookingsByTimeRange".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getFoodBookingsByTimeRange", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getFoodBookingsByTimeRange"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_startTime
                                .try_into_vm_value(&mut boa_context)?,
                            _cdk_user_defined_endTime
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::update(name = "addFoodBooking")]
#[candid::candid_method(update, rename = "addFoodBooking")]
async fn _cdk_user_defined_addFoodBooking(
    _cdk_user_defined_payload: FoodBookingPayload,
) -> (_AzleResult<FoodBooking, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addFoodBooking".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addFoodBooking", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addFoodBooking"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateFoodBooking")]
#[candid::candid_method(update, rename = "updateFoodBooking")]
async fn _cdk_user_defined_updateFoodBooking(
    _cdk_user_defined_id: String,
    _cdk_user_defined_payload: FoodBookingPayload,
) -> (_AzleResult<FoodBooking, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateFoodBooking".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateFoodBooking", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateFoodBooking"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "deleteFoodBooking")]
#[candid::candid_method(update, rename = "deleteFoodBooking")]
async fn _cdk_user_defined_deleteFoodBooking(
    _cdk_user_defined_id: String,
) -> (_AzleResult<FoodBooking, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deleteFoodBooking".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deleteFoodBooking", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deleteFoodBooking"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "markFoodBookingAsDelivered")]
#[candid::candid_method(update, rename = "markFoodBookingAsDelivered")]
async fn _cdk_user_defined_markFoodBookingAsDelivered(
    _cdk_user_defined_id: String,
) -> (_AzleResult<FoodBooking, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "markFoodBookingAsDelivered".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("markFoodBookingAsDelivered", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "markFoodBookingAsDelivered"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
struct FoodBooking {
    id: Box<String>,
    foodName: Box<String>,
    quantity: Box<_CdkFloat64>,
    deliveryAddress: Box<String>,
    createdAt: Box<u64>,
